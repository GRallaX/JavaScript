...
function returArguments() {
  for (var arg of arguments) {
    console.log("Аргумент функции: " + arg);
  }
  console.log("Функция: " + arguments.callee);
}
...
function getCurrentPostComments(postId) {
  ...
  for (let comment in comments) {
    {
      if (postId == comments[comment].postId) {
        for (let author in users) {
          if (author == comments[comment].author) {
            res.push({
